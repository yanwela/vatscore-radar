...
# ==============================================================================
# GLOBAL DYNAMIC REFRESH ENGINE & STATE SECURITY LOCK
# ==============================================================================
...
# GÖRÜNMEZ SAAT MOTORU VE AUTOMATED RERUN TETİKLEYİCİSİ
js_timer_html = """
<script>
    ...
    setTimeout(sendTimeAndRefresh, 200); 
    setInterval(sendTimeAndRefresh, 30000);
</script>
"""
...
if data:
    ...
    fir_pilots = []
    js_ready_pilots_list = [] # JAVASCRIPT'E GÖNDERİLECEK TAM KORUNMUŞ HAM DATA LİSTESİ
    ...
    # VERİLERİN AYIKLANDIĞI VE HESAPLANDIĞI ANA DÖNGÜ (ASLA DOKUNULMADI)
    for p in pilots:
        ...
        if matches_flight_plan or is_physically_here:
            ...
            # Python dataframe ve grafikler için veri
            fir_pilots.append({
            ...
            # JS'e ham objeyi kayıpsız paslamak için saklıyoruz
    ...
    with tab2:
        ...
        if fir_pilots:
            ...
            st.info(f"Showing {len(doc_fir)} active aircraft tracks inside {st.session_state.current_fir_prefix}. Click a row to inspect telemetry.")
            ...
            <style>
                ...
                /* INLINE FLIGHT RULES BOX - TAM İSTEDİĞİN GÖRSELDEKİ TASARIM */
                ...
                .inline-rules-box.ifr {
                    background-color: #1d3531;
                    color: #22c55e;
                    border: 1px solid #22c55e40;
                }
                .inline-rules-box.vfr {
            ...
            <script>
                ...
                function updateHaversineProgressMetrics(depIcao, arrIcao, currentLat, currentLon) {
                    ...
                    if (!depPoint || !arrPoint) {
                        txtBox.innerText = "Coordinates Missing";
                ...
                function classifyAircraftLocal(acType, callsign) {
                    ...
                    if (["F16", "F18", "F15", "F35", "C17", "A400"].includes(acType)) return "⚔️ Military";
                    if (callsign.startsWith("TUR") || callsign.startsWith("RCH") || callsign.includes("MIL")) return "⚔️ Military";
                    if (["C172", "C152", "DA42"].includes(acType)) return "🛩️ General Aviation";
                    return "✈️ Commercial";
                }
                ...
                function buildTable(pilotsList) {
                    ...
                    // BURASI PYTHON TARAFINDA FİLTRELENMİŞ VERİYİ DOĞRUDAN BASAR (BOŞ KALMA SORUNU ÇÖZÜLDÜ)
                ...
                function openDossier(callsign) {
                    ...
                    if(p.rules === "VFR") {
                        ...
                        badge.style.backgroundColor = "#1d3531"; badge.style.color = "#22c55e"; badge.style.borderColor = "#22c55e40";
                    }
                ...
                if (autoOpenCallsign) setTimeout(() => { openDossier(autoOpenCallsign); }, 250);
            </script>
            """
