...
if data:
    ...
    with tab2:
        ...
        if fir_pilots:
            ...
            # Pure Streamlit table display (no iframe)
            active_cols = ["Callsign"] + [c for c in st.session_state.visible_columns if c in doc_fir.columns]
            display_df = doc_fir[active_cols].copy()
            
            st.markdown("#### 📊 Active Aircraft in Region")
            st.dataframe(
                display_df,
                height=400,
                use_container_width=True,
                column_config={
                    "Callsign": st.column_config.TextColumn("✈️ Callsign", width=120),
                    "Category": st.column_config.TextColumn("🎭 Category"),
                    "Altitude (FT)": st.column_config.NumberColumn("🛫 Alt (FT)", format="%.0f"),
                    "Speed (KT)": st.column_config.NumberColumn("⚡ Speed (KT)", format="%.0f"),
                }
            )
            
            # Telemetry Dossier Modal (Streamlit native)
            if "selected_pilot_callsign" not in st.session_state:
                st.session_state.selected_pilot_callsign = ""
            
            # Quick access buttons
            st.markdown("**Click a callsign below to view telemetry:**")
            button_cols = st.columns(min(len(display_df), 5))
            for idx, (_, row) in enumerate(display_df.iterrows()):
                with button_cols[idx % len(button_cols)]:
                    if st.button(f"🛫 {row['Callsign']}", key=f"pilot_btn_{idx}"):
                        st.session_state.selected_pilot_callsign = row['Callsign']
            
            # Display telemetry if a pilot is selected
            if st.session_state.selected_pilot_callsign:
                # Find pilot data
                pilot_callsign = st.session_state.selected_pilot_callsign
                pilot_data = None
                for p in pilots:
                    if p.get("callsign", "").upper() == pilot_callsign.upper():
                        pilot_data = p
                        break
                
                if pilot_data:
                    with st.container(border=True):
                        st.markdown(f"### 🛰️ Telemetry Dossier — **{pilot_callsign}**")
                        
                        col_close = st.columns([20, 1])[1]
                        with col_close:
                            if st.button("✕ Close", key="close_telemetry"):
                                st.session_state.selected_pilot_callsign = ""
                                st.rerun()
